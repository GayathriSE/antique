    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          ...
          className="bg-amber-700 hover:bg-amber-800 text-white font-semibold px-4 py-2 rounded-lg text-sm transition-colors"
        >
      ...
      {addresses.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {addresses.map((addr: any) => (
            <div
              key={addr.id}
              className={`bg-white rounded-xl border p-5 ${addr.isDefault ? "border-amber-400" : "border-stone-200"}`}
            >
              {addr.isDefault && (
                <span className="text-xs bg-amber-100 text-amber-700 font-semibold px-2 py-0.5 rounded-full mb-3 inline-block">
                  Default
                </span>
              )}
